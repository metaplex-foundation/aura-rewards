...
async fn success() {
    ...
    test_rewards
        .deposit_mining(&mut context, &mining, 100, lockup_period, &mining_owner)
        .await
        .unwrap();

    test_rewards
        .withdraw_mining(&mut context, &mining, 30, &mining_owner.pubkey())
        .await
        .unwrap();
...
async fn success_with_5kkk_after_expiring() {
    ...
    test_rewards
        .withdraw_mining(&mut context, &mining, 5000000000, &mining_owner.pubkey())
        .await
        .unwrap();
